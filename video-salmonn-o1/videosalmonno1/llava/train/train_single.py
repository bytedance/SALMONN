...
def train():
    ...
    if training_args.run_test or training_args.do_demo:
        ...
        if training_args.test_output_dir is not None:
            ...
            hdfs_path = "hdfs://harunava/home/byte_data_seed_azureb_tteng/user/guangzhisun/test"
            hmkdir(os.path.join(hdfs_path, test_output_dir))
            hmkdir(os.path.join(hdfs_path, f"{test_output_dir}/train_config"))
        ...
        if training_args.test_output_dir is not None:
            ...
            output_path = os.path.join(test_output_dir, f"test_results_rank{data_args.current_device}.json")
            with open(output_path, 'w') as fp:
                json.dump(results, fp)
            print(f"rank {data_args.current_device} start upload")
            hcopy(output_path, os.path.join("hdfs://harunava/home/byte_data_seed_azureb_tteng/user/guangzhisun/test", test_output_dir, f"test_results_rank{data_args.current_device}.json"), chunk_thread_num=1)
            print(f"rank {data_args.current_device} finish upload")
