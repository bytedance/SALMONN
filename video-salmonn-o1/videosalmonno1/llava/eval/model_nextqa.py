...
def parse_args():
    ...
    parser.add_argument("--mapping_file", help="Path to the ground truth file containing question.", required=True)
...
def run_inference(args):
    ...
    for sample in tqdm(gt_questions):
        ...
        question = f'{sample["question"]}?\nAnswer the question using at most five words or phrase.'
        qid = sample["qid"]
        ...
        with torch.inference_mode():
            ...
            if "mistral" in args.model_path:
                ...
                output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=True, temperature=0.2, max_new_tokens=1024, use_cache=True, stopping_criteria=[stopping_criteria])
                # output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=False, temperature=0, max_new_tokens=64, top_p=1.0, num_beams=1,use_cache=True, stopping_criteria=[stopping_criteria])
