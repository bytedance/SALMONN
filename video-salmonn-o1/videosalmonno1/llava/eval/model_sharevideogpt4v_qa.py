...
def parse_args():
    ...
    parser.add_argument("--mm_newline_position", type=str, default="gird")
    parser.add_argument("--mm_resampler_location", type=str, default="before")
...
def load_video(video_path, args):
    ...
    num_frames_to_sample = args.for_get_frames_num if len(frame_files) >= args.for_get_frames_num else len(frame_files)
...
def run_inference(args):
    ...
    if "gpt4v" != args.model_path:
        ...
        if args.overwrite == True:
            ...
            overwrite_config["mm_pooling_position"] = args.mm_resampler_location
    ...
    with open(args.gt_file) as file:
        gt_questions = [json.loads(line) for line in file]
    ...
    if os.path.exists(answers_file):
        ...
        for _ in current_answers_list:
            video_id = _["id"]
            current_answers_json[video_id] = _
    ...
    for sample in tqdm(gt_questions):
        ...
        if video_id in current_answers_json:
            continue
        # import pdb;pdb.set_trace()
        question = sample["conversations"][0]["value"].replace("<video>\n", "")

        # question = f"Answer me one question of this video:\n{question}" #\nAnswer the question in a clear and concise style."
        answer = sample["conversations"][1]["value"]
        caption =  sample["caption"]

        sample_set = {"id":video_id,"question": question, "answer": answer, "video_name": video_name, "caption": caption}
        ...
        if "gpt4v" != args.model_path:
            with torch.inference_mode():
                ...
                if "mistral" in args.model_path:
                    ...
                    # output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=True, temperature=0.2, max_new_tokens=1024, use_cache=True, stopping_criteria=[stopping_criteria])
                    output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=False, temperature=0, max_new_tokens=1024, top_p=1.0, num_beams=1,use_cache=True, stopping_criteria=[stopping_criteria])
