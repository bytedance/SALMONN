...
class LazyAVSupervisedDataset(Dataset):
            ...
            if self.data_args.use_tos:
                for _ in range(100):
                    try:
                        ...
                        assert len(image_data.data) == int(image_data.headers['Content-Length'])
                    except:
                        continue
                    break
    ...
    def save_tt(self, vr, output_video_path):
        try:
            fps = vr.get_avg_fps()
            frame_count = len(vr)
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0]:
            ...
            elif 'video' in sources[0] or "tos_key" in sources[0] or "tt_id" in sources[0]:
                ...
                if suffix == 'pkl':
                    ...
                    try:
                        if True:
                            if "tos_key" in sources[0] and "shareVideoGPTV" in sources[0]["tos_key"]:
                                for _ in range(3):
                                    try:
                                ...
                                total_frame_num = len(all_frames)
                                real_time = total_frame_num * 1.0

                                # num_frames_to_sample = 10
                                avg_fps = max(round(ori_fps / self.data_args.video_fps), 1)
                                frame_idx = [i for i in range(0, total_frame_num, round(avg_fps))]
                                ...
                                # sampled_indices = np.linspace(0, total_frame_num - 1, round(num_frames_to_sample * self.data_args.video_fps), dtype=int)

                                # print(len(frame_idx), total_frame_num, ori_fps, self.data_args.video_fps, real_time)
                            ...
                            elif "tt_id" in sources[0] or "tos_key" in sources[0]:
                                if "tt_id" in sources[0]:
                            ...
                            if isinstance(video, list):
                                ...
                                # if hasattr(self.data_args, "retrieve_image_processor"):
                                #     sources[0]["raw_video"] = self.data_args.retrieve_image_processor.image_processor.preprocess(
            ...
            if 'tos_audio' in sources[0] and sources[0]['tos_audio']:
                audio_file = sources[0]['tos_key']
                # resp = self.cli.get_object(audio_file)
                # video_data = resp.data
                out, _ = (ffmpeg.input('pipe:0', format='mp4').output('pipe:1', format='wav', ac=1, ar='16k').run(input=video_data, capture_stdout=True, capture_stderr=True))
                ...
                if "timestamps" in sources[0]:
                    ...
                    audio_file = audio_file + "_" + str(sources[0]["timestamps"][0]) + "-" + str(sources[0]["timestamps"][1])
