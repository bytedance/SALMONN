...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # vision_resampler.mm_projector = self.mm_projector
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            if self.config.mm_resampler_type != "vlm_attention":
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images,input_modality="image",prompts=None, image_counts=None, long_video=False):
        ...
        if input_modality == "video":
            if self.config.mm_resampler_type != "vlm_attention":
                ...
                image_features = self.get_model().vision_resampler(image_features, prompts=prompts, image_counts=image_counts, long_video=long_video)
        else:
            image_features = self.get_model().mm_projector(image_features)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            # import pdb; pdb.set_trace()
            if self.config.mm_resampler_type in ["qformer"]:
                image_features = self.encode_images(concat_images,"video", prompts, image_counts, long_video=long_video)
            else:
            ...
            if mm_patch_merge_type == 'flat':
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx not in image_idx_in_batch:
                        if self.config.mm_resampler_type in ["vlm_attention"]:
                            new_image_features.append(image_feature)
                        else:
                            # import pdb; pdb.set_trace()
                            if self.config.mm_resampler_type in ["spatial_pool"] and self.config.patchify_video_feature:
            ...
            if self.config.mm_resampler_type in ["qformer"]:
                image_features = self.encode_images(images,"video")
            else:
