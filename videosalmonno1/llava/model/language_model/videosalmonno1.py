...
class videoSALMONNo1Config(Qwen2Config):
    model_type = "videosalmonno1"


class videoSALMONNo1Model(LlavaMetaModel, Qwen2Model):
    config_class = videoSALMONNo1Config

    def __init__(self, config: Qwen2Config):
        super(videoSALMONNo1Model, self).__init__(config)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if getattr(self, 'mm_projector', None) is None or self.multi_frame_projector:
            if False:
                pass
            elif self.multi_frame_projector:
...
class videoSALMONNo1ForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    config_class = videoSALMONNo1Config

    def __init__(self, config, **audio_config):
        Qwen2ForCausalLM.__init__(self, config)
        config.model_type = "videosalmonno1"
        config.rope_scaling = None
        self.vocab_size = config.vocab_size

        self.add_time_token = config.add_time_token

        self.model = videoSALMONNo1Model(config)
...
AutoConfig.register("videosalmonno1", videoSALMONNo1Config)
AutoModelForCausalLM.register(videoSALMONNo1Config, videoSALMONNo1ForCausalLM)
