...
from qwenvl.model.ttt.ops import ttt_linear, ttt_mlp, ttt_mlp_cg
...
class SSMGating(nn.Module):
    ...
    def forward(self, x):
        ...
        # gating_alpha = torch.sigmoid(gating_alpha)
        gating_alpha = torch.tanh(gating_alpha)
        return gating_alpha


class TTTWrapper(nn.Module):
    def __init__(self, config: ModelConfig, discard_V=False):
        super().__init__()
        ...
        if config.ssm_layer == "ttt_linear":
            ...
            self.ttt = TTTMLPCG(config, CG_max_iterations=config.cg_max_iter, ttt_hidden_size=self.ttt_hidden_size)
        else:
...
class TTTBase(nn.Module):
    ...
    def get_qkv_projections(self, hidden_states):
        if "lite" in self.slot_type:
            XQ = self.wq(hidden_states)
            XK, XV = hidden_states, hidden_states
        else:
            XQ, XK, XV = (
    ...
    def forward(
        ...
        hidden_states = self.process_input(hidden_states, freqs_cis, seq_metadata)
    ...
    def process_input(self, hidden_states: torch.Tensor, freqs_cis: torch.Tensor, seq_metadata: SequenceMetadata):  
        ...
        if self.predict_lag:
            ...
            if "forward" in self.slot_type:
                ...
                new_hidden_states = torch.cat([hidden_states.new_zeros(B, lag_distance, D), hidden_states[:, :-lag_distance]], dim=1)
                lag_loss_mask = torch.cat([hidden_states.new_zeros(lag_distance), hidden_states.new_ones(L-lag_distance)], dim=0)
            else:
        ...
        if "lite" not in self.slot_type:
            XV = self.wk(new_hidden_states)
            XK = self.wv(hidden_states)
        ...
        hidden_states, XQ, XK, XV = self.reshape_to_mini_batch(hidden_states, XQ, XK, XV)
        if self.predict_lag:
            lag_loss_mask = lag_loss_mask.reshape(-1, mini_batch_size).unsqueeze(0).unsqueeze(0).unsqueeze(-1) # (B, H, N, bsize, 1)
    ...
    def ttt(self, inputs, downsample_ids=None, text_query=None, allstates=None):
        ...
        if self.downsample_option == "none":
            states, XQW_batch = ttt_mlp_cg(
                ...
                return_loss=True if "loss" in self.slot_type else False
            )
        if "losses" in states:
            states["losses"] = torch.cat(states["losses"], dim=-1)
        XQW_batch = XQW_batch.reshape(B, L, self.width)
