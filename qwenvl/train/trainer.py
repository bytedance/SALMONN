...
class QwenVLTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if train_type == "sft":
            if "distill" in model.module.model.model.ttt_type:
                with self.accelerator.unwrap_model(self.model).disable_adapter():
                    distill_inputs = {k: v for k, v in inputs.items()}
                    distill_inputs["pixel_values_videos"] = inputs["distill_pixel_values_videos"]
                    distill_inputs["video_grid_thw"] = inputs["distill_video_grid_thw"]
                    distill_inputs["input_ids"] = inputs["distill_input_ids"]
                    distill_inputs["labels"] = None
                    distill_inputs["distillround"] = True
                    distill_inputs["attention_mask"] = inputs["distill_attention_mask"]
                    with torch.no_grad():
                        distill_out = model(**distill_inputs)
                        distill_labels = inputs["distill_labels"][0, 1:]
                        start_idx = torch.where((distill_labels != -100)==True)[0][0]
                        distill_labels = distill_labels[start_idx:]
                        soft_targets = torch.softmax(distill_out.logits[0, :-1][start_idx:], dim=-1)
                        lossmask = distill_labels != -100
                    torch.cuda.empty_cache()
                inputs["return_logits"] = True
                outputs = model(**inputs)
                logp = torch.log_softmax(outputs.logits[0, :-1], dim=-1)
                distill_loss = ((-soft_targets * logp).sum(dim=-1) * lossmask).sum() / (lossmask.sum())
                outputs.loss = outputs.loss + self.args.distill_factor * distill_loss
            else:
                outputs = model(**inputs)
        elif train_type == "dpo":
    ...
    def validate(self):
        ...
        test_dataloader = DataLoader(self.eval_dataset, batch_size=1, collate_fn=self.data_collator, num_workers=num_workers)
